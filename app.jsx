...
const { ResponsiveContainer, ComposedChart, Bar, Line, XAxis, YAxis, Tooltip, ReferenceLine } = Recharts;
...
function defaultQty(sym) { return sym === "BTC" ? 0.01 : COINS[sym]?.price > 100 ? 0.1 : COINS[sym]?.price > 1 ? 1 : 100; }
...
function App() {
  ...
  const initCard = () => { const s = {}; STRATS.forEach(st => { s[st.id] = { symbol: st.ds, value: st.def, qty: defaultQty(st.ds) }; }); return s; };
            ...
            return (
              <div key={sym} className="sr" onClick={() => { setSelected(sym); setTab("chart"); }}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontFamily: "var(--m)", fontWeight: 700, fontSize: 15, color: symColor(sym) }}>{sym}</div>
                ...
                <div style={{ display: "grid", gap: 10 }}>
                  {Object.entries(holdings).map(([sym, h]) => {
                    ...
                    return (<div key={sym} style={{ background: "#0f172a", borderRadius: 8, padding: "14px 16px", border: "1px solid rgba(255,255,255,0.04)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <span style={{ fontFamily: "var(--m)", fontWeight: 700, fontSize: 18, color: symColor(sym) }}>{sym}</span>
